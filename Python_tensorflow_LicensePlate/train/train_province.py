...
class trainProvince:
    def __init__(self):
        ...
        self.WIDTH = 32
        self.HEIGHT = 40
        self.iterations_num = 400

        self.SAVER_DIR = "../resources/model/province/"
        self.TRAIN_DIR = "../resources/train-images/training-set/chinese-characters/"
        self.VALIDATION_DIR = "../resources/train-images/validation-set/chinese-characters/"
        ...
        self.province_labels = ['川', '鄂', '赣', '甘', '贵', '桂', '黑', '沪', '冀', '津', '京', '吉', '辽',
                                ...
                                '渝', '粤', '云', '藏', '浙']
        self.province_Train_Dirs = ['zh_cuan', 'zh_e', 'zh_gan', 'zh_gan1', 'zh_gui', 'zh_gui1', 'zh_hei',
    ...
    def train(self):
        ...
        with tf.Session() as sess:
            ...
            # readout层
            W_fc2 = tf.Variable(tf.truncated_normal([512, self.provice_NUM_CLASSES], stddev=0.1), name="W_fc2")
            b_fc2 = tf.Variable(tf.constant(0.1, shape=[self.provice_NUM_CLASSES]), name="b_fc2")
            ...
            batch_size = 60
            iterations = self.iterations_num
    ...
    def predict(self):
        with tf.Session() as sess3:
            ...
            for n in range(1, 2):
                ...
                result = sess3.run(conv, feed_dict={self.x: np.array(img_data), keep_prob: 1.0})
                max1 = 0
                max2 = 0
                max3 = 0
                max1_index = 0
                ...
                print("概率：  [%s %0.2f%%]    [%s %0.2f%%]    [%s %0.2f%%]" % (
                    self.province_labels[max1_index], max1 * 100, self.province_labels[max2_index], max2 * 100, self.province_labels[max3_index], max3 * 100))
