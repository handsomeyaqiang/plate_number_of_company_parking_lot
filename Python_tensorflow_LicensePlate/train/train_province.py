...
y_ = tf.placeholder(tf.float32, shape=[None, provice_NUM_CLASSES])
...
def train():
    ...
    val_labels = np.array([[0] * provice_NUM_CLASSES for i in range(val_count)])
    ...
    with tf.Session() as sess:
        ...
        # readout层
        W_fc2 = tf.Variable(tf.truncated_normal([512, provice_NUM_CLASSES], stddev=0.1), name="W_fc2")
        b_fc2 = tf.Variable(tf.constant(0.1, shape=[provice_NUM_CLASSES]), name="b_fc2")
