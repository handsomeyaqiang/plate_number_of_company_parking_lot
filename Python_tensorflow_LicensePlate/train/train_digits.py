...
y_ = tf.placeholder(tf.float32, shape=[None, digit_NUM_CLASSES])
...
def train_license():
    ...
    input_labels = np.array([[0] * digit_NUM_CLASSES for i in range(input_count)])
    ...
    val_labels = np.array([[0] * digit_NUM_CLASSES for i in range(val_count)])
    ...
    with tf.Session() as sess:
        ...
        # readout层
        W_fc2 = tf.Variable(tf.truncated_normal([512, digit_NUM_CLASSES], stddev=0.1), name="W_fc2")
        b_fc2 = tf.Variable(tf.constant(0.1, shape=[digit_NUM_CLASSES]), name="b_fc2")
...
def predict():
    ...
    with sess2.as_default():
        ...
        for n in range(3, 8):
            ...
            max3_index = 0
            # 获取前三个概率最大的字符及其索引
            for j in range(digit_NUM_CLASSES):
                if result[0][j] > max1:
                    max1 = result[0][j]
                    max1_index = j
                    continue
            ...
            print("概率：  [%s %0.2f%%]    [%s %0.2f%%]    [%s %0.2f%%]" % (
                digit_lables[max1_index], max1 * 100, digit_lables[max2_index], max2 * 100, digit_lables[max3_index], max3 * 100))
            print("车牌号" + str(n) + "是: %s" % digit_lables[max_license_index])
            license_num += digit_lables[max_license_index]
            sess2.close()
    return license_num
