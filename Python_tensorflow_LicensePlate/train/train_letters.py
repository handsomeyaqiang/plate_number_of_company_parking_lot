...
class trainLetter:
    def __init__(self):
        ...
        self.WIDTH = 32
        self.HEIGHT = 40
        self.iterations_num = 500
        self.SAVER_DIR = "../resources/model/letter/"
        self.TRAIN_DIR = "../resources/train-images/training-set/letters/"
    ...
    def predict(self):
        with tf.Session() as sess1:
            ...
            for n in range(2, 3):
                ...
                result = sess1.run(conv, feed_dict={self.x: np.array(img_data), keep_prob: 1.0})
                ...
                print("概率：  [%s %0.2f%%]    [%s %0.2f%%]    [%s %0.2f%%]" % (
                    self.letter_lables[max1_index], max1 * 100, self.letter_lables[max2_index], max2 * 100, self.letter_lables[max3_index],
                max3 * 100))
